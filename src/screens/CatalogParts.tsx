...
const CatalogParts: React.FC = () => {
  ...
  // 3. Real-time sync with onSnapshot for pecas (Constraint 3)
  useEffect(() => {
    console.log("Inicializando onSnapshot para peças...");
    const unsub = onSnapshot(collection(db, 'pecas'), (snap) => {
      const list = snap.docs.map(doc => {
        ...
        return {
          ...
          suggestedPrice: Number(data.valorVenda !== undefined ? data.valorVenda : (data.suggestedPrice || 0)),
      ...
      console.log("Peças carregadas:", list);
      setParts(list);
    }, (error) => {
      console.error("Erro completo ao buscar peças do Firestore:", error);
  ...
  // Unique categories
  const categories = ['Todas', ...Array.from(new Set(parts.map(p => p.category).filter(Boolean)))];

  const handleDelete = async (id: string) => {
    if (window.confirm("Deseja realmente excluir esta peça do Firestore em tempo real?")) {
      try {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const valorVenda = Number(formData.get('valorVenda'));
    const valorCusto = Number(formData.get('valorCusto')) || 0;
    ...
    const documentData = {
      ...
      suggestedPrice: valorVenda,
      valorCusto: valorCusto,
      costPrice: valorCusto,
      estoque: estoque,
      stock: estoque,
      marca: brand,
      brand: brand,
    ...
    try {
      await setDoc(doc(db, 'pecas', itemId), documentData);
      
      // Show success, do NOT close form (Requirement 5)
      setSuccessMsg("Peça cadastrada com sucesso!");
      ...
      if (editingItem) {
        ...
        // Reset only values inside the form to allow rapid successive additions
    ...
    <div className="p-4 md:p-8 max-w-7xl mx-auto min-h-screen">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12 pt-4">
        <div>
          ...
          <p className="text-white/40 font-bold uppercase tracking-widest text-[10px] mt-2">
            Peças e insumos cadastrados no Firestore atualizados em tempo real
          </p>
        </div>
      ...
      <div className="flex flex-col md:flex-row gap-6 mb-10">
        <div className="flex-1 relative group">
          ...
          <input 
            type="text" 
            placeholder="Pesquisar peça por nome, código ou fabricante..."
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          ...
          >
            <div>
              <div className="flex justify-between items-start mb-6">
                <span className="text-[9px] px-3 py-1.5 rounded-xl font-black uppercase tracking-widest italic shadow-sm border bg-black text-primary border-primary/20">
                  {item.category || 'Peças'}
                </span>
              ...
              <div className="mb-6">
                <div className="flex items-center justify-between mb-1">
                  <span className="text-[8px] tracking-wide font-mono text-white/30 uppercase">{item.brand || 'Original'}</span>
                  {item.code && <span className="text-[8px] font-mono bg-white/5 text-primary px-2 py-0.5 rounded">COD: {item.code}</span>}
                </div>
                <h3 className="font-black text-2xl leading-tight uppercase italic tracking-tighter text-white group-hover:text-primary transition-colors">
                  {item.name}
                </h3>
                <p className="text-xs text-white/40 line-clamp-3 leading-relaxed mt-2">
            ...
            <div className="pt-6 border-t border-white/5 flex flex-col gap-4">
              <div className="flex items-center justify-between">
                <span className="text-[9px] font-black uppercase text-white/20 tracking-[0.3em]">ESTOQUE:</span>
                <span className={`text-xs font-black ${Number(item.stock || 0) > 2 ? 'text-green-400' : 'text-orange-500 animate-pulse'}`}>
                  {item.stock || 0} UNIDADES
                </span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-[9px] font-black uppercase text-white/20 tracking-[0.3em]">PREÇO DE VENDA:</span>
              ...
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  alert(`Para aplicar a peça "${item.name}" a uma Ordem de Serviço, abra a tela de "Ordens de Serviço", inicie ou edite um registro e utilize a busca integrada.`);
                }}
        ...
        {filteredParts.length === 0 && (
          <div className="col-span-full py-24 text-center border-2 border-dashed border-white/5 rounded-[2.5rem]">
            ...
            <p className="text-white/30 font-black uppercase tracking-[0.2em] text-xs">Nenhuma peça cadastrada no pátio.</p>
          </div>
        )}
      </div>

      {/* Form Dialog Modal (Requirement 5) */}
      <AnimatePresence>
        {isModalOpen && (
          <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/90 backdrop-blur-md">
            ...
            >
              ...
              <h2 className="text-2xl font-black italic uppercase tracking-tight text-white mb-6">
                {editingItem ? 'Editar' : 'Cadastrar'} <span className="text-primary">Peça</span>
              </h2>

              <form onSubmit={handleSubmit} className="space-y-5">
                <div>
                  <label className="block text-[10px] font-black text-white/40 uppercase tracking-widest mb-1.5">Nome da Peça / Componente *</label>
                  <input 
                    required 
                    type="text" 
                    name="name" 
                    placeholder="Ex: Amortecedor Dianteiro Cofap"
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-[10px] font-black text-white/40 uppercase tracking-widest mb-1.5">Categoria *</label>
                    <input 
                      required 
                      type="text" 
                      name="category" 
                  ...
                  <div>
                    <label className="block text-[10px] font-black text-white/40 uppercase tracking-widest mb-1.5">Fabricante / Marca</label>
                    <input 
                      type="text" 
                      name="brand" 
                ...
                <div className="grid grid-cols-3 gap-3">
                  <div>
                    <label className="block text-[9px] font-black text-white/40 uppercase tracking-widest mb-1.5">Custo R$</label>
                    <input 
                      type="number" 
                      step="0.01"
                      name="valorCusto" 
                      ...
                      className="input-field w-full px-3 py-3 bg-black/50 border border-white/10 text-center rounded-xl text-white text-xs focus:outline-none focus:border-primary"
                    />
                  </div>
                  <div>
                    <label className="block text-[9px] font-black text-white/40 uppercase tracking-widest mb-1.5">Venda R$ *</label>
                    <input 
                      required
                      type="number" 
                      step="0.01"
                  ...
                  <div>
                    <label className="block text-[9px] font-black text-white/40 uppercase tracking-widest mb-1.5">Quantidade Estoque</label>
                    <input 
                      type="number" 
                      name="estoque" 
                      placeholder="5"
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-[10px] font-black text-white/40 uppercase tracking-widest mb-1.5">Código de Fábrica (Part Number)</label>
                    <input 
                      type="text" 
                      name="code" 
                      placeholder="Ex: COF-9021-X"
                      defaultValue={editingItem?.code}
                      className="input-field w-full px-4 py-3 bg-black/50 border border-white/10 rounded-xl text-white text-sm focus:outline-none focus:border-primary"
                    />
                  </div>
                  <div>
                    <label className="block text-[10px] font-black text-white/40 uppercase tracking-widest mb-1.5">Especificações / Notas</label>
                    <input 
                      type="text" 
                      name="description" 
                      placeholder="Ex: Lado direito dianteiro..."
